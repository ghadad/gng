...
const CouchdbDAL = class {
  ...
  async renameId(db, sourceId, rev, destId, options = {}) {
    ...
    try {
      ...
      throw new Error(
        `Falied to copy  ${db} -> ${sourceId} ->  ${destId} , reason :  ${e.message}`
      );
    }
  }
  ...
  findAsStream(db, query = {}) {
    const dbref = this.nano.db.use(db);
    return dbref.findAsStream(query).pipe(JSONStream.parse("docs.*"));
  }
  ...
  async upsertBulk(db, data, options = {}) {
    if (options.checkExist) {
      for (let doc of data) {
        if (doc._id) {
          let existsDoc = await this.get(db, doc._id).catch((e) =>
            console.log(e)
          );
