...
const usernameParam = S.object().prop("query", S.string().required());

const ids = S.object().prop("ids", S.array().items(S.string()));
...
const avatarInput = S.object().prop("avatar_url", S.string().required());
...
const resultOk = S.object().prop("result", S.string().required());
...
const wsConnection = S.object().prop("op").prop("data", connectionList);
