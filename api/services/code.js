...
class CodeService {
  ...
  async get(username, type, token) {
    const codeFilter = new CodeFilter(username, type, token).getFilter();
    const record = await Code.findOne(codeFilter).exec();
    return record;
  }
  ...
  async create(username, type) {
    ...
    const validto = new Date(
      +new Date() + config.authSettings.codeValidityInMinutes * 60 * 1000
    );
    const data = {
      username,
      token,
